// Name: Vamshi Gopari    G#: G01355694   Homework 4 AVL Tree
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <math.h>
...
int balanceFactor(struct address_t *node);
void updateHeightAndBalanceFactor(struct address_t *node);
struct address_t *rebalance(struct address_t *node);
struct address_t *rotateLeft(struct address_t *node);
struct address_t *rotateRight(struct address_t *node);
struct address_t *rotateLeftThenRight(struct address_t *node);
struct address_t *rotateRightThenLeft(struct address_t *node);
void insertNode(struct address_t **root, int *octet, char *alias, struct address_t *parent);
struct address_t *newNode(int *octet, char *alias, struct address_t *parent);
int compareOctets(int *octet1, int *octet2);
...
int balanceFactor(struct address_t *node)
{
  ...
  {
    return (height(node->leftChild) - 1) - (height(node->rightChild) - 1);
  }
}

// Update the height and balance of a node based on its children
void updateHeightAndBalanceFactor(struct address_t *node)
{
  int leftHeight = node->leftChild ? node->leftChild->height - 1 : -1;
  int rightHeight = node->rightChild ? node->rightChild->height - 1 : -1;
  node->height = (leftHeight > rightHeight ? leftHeight : rightHeight) + 2;
  node->balance = balanceFactor(node);
}

// Rebalance a node if necessary
struct address_t *rebalance(struct address_t *rootNode)
{
  updateHeightAndBalanceFactor(rootNode);
  if (rootNode->balance > 1)
  {
    if (balanceFactor(rootNode->leftChild) < 0)
    {
      rootNode->leftChild = rotateLeft(rootNode->leftChild);
    }
    rootNode = rotateRight(rootNode);
  }
  else if (rootNode->balance < -1)
  {
    if (balanceFactor(rootNode->rightChild) > 0)
    {
  ...
  if (rootNode->balance > 1 && balanceFactor(rootNode->leftChild) > 0)
  {
    rootNode = rotateLeftThenRight(rootNode);
  }

  // Check if right-left rotation is needed
  if (rootNode->balance < -1 && balanceFactor(rootNode->rightChild) < 0)
  {
    rootNode = rotateRightThenLeft(rootNode);
  }
  return rootNode;
}
...
struct address_t *rotateLeft(struct address_t *node)
{
  struct address_t *newRoot = node->rightChild;
...
struct address_t *rotateRight(struct address_t *node)
{
  struct address_t *newRoot = node->leftChild;
...
void readAndInitFile()
{
  ...
  if (fp == NULL)
  {
    printf("Failed to open file %s", "CS531_Inet.txt");
  }

  // Read each line of file and insert node into tree
  while (fgets(line, sizeof(line), fp) != NULL)
  {
    sscanf(line, "%d.%d.%d.%d %s", &octet[0], &octet[1], &octet[2], &octet[3], alias);
    insertNode(&rootNode, octet, alias, NULL);
    nodeCount++;
  }
...
void insertNode(struct address_t **rootNode, int *octet, char *alias, struct address_t *parent)
{
  // If root is null, create new node and set it as root
  if (*rootNode == NULL)
  {
    *rootNode = newNode(octet, alias, parent);
    return;
  }

  // If alias already exists in tree, update IP address and return
  int cmp = strcmp(alias, (*rootNode)->alias);
  if (cmp == 0)
  {
    memcpy((*rootNode)->octet, octet, sizeof(int) * 4);
    updateHeightAndBalanceFactor(*rootNode);
    *rootNode = rebalance(*rootNode);
    nodeCount++;
  }

  // Recursively insert node into left or right subtree
  if (cmp < 0)
  {
    insertNode(&(*rootNode)->leftChild, octet, alias, *rootNode);
  }
  else
  {
    insertNode(&(*rootNode)->rightChild, octet, alias, *rootNode);
  }
...
// Create a new node with given IP address and alias
struct address_t *newNode(int *octet, char *alias, struct address_t *parent)
{
  struct address_t *node = (struct address_t *)malloc(sizeof(struct address_t));
  memcpy(node->octet, octet, sizeof(int) * 4);
  strncpy(node->alias, alias, sizeof(node->alias));
  node->height = 1;
  node->balance = 0;
  node->leftChild = NULL;
...
void addAddress()
{
  if (rootNode != NULL)
  {
    ...
    if (isIPAddressValid == 1 && isIPAddressDuplicate == 1)
    {
      ...
      // Update height and balance factor of nodes and balance tree
      updateHeightAndBalanceFactor(currentNode);
...
// performs delete action on a node
struct address_t *deleteNode(struct address_t *rootNode, char alias[])
{
  if (rootNode == NULL)
  {
    return rootNode;
  }
  if (strcmp(alias, rootNode->alias) < 0)
  {
    rootNode->leftChild = deleteNode(rootNode->leftChild, alias);
  }
  else if (strcmp(alias, rootNode->alias) > 0)
  {
    rootNode->rightChild = deleteNode(rootNode->rightChild, alias);
  }
  else
  {
    ...
    else
    {
      ...
      rootNode->octet[0] = temp->octet[0];
      rootNode->octet[1] = temp->octet[1];
      rootNode->octet[2] = temp->octet[2];
      rootNode->octet[3] = temp->octet[3];
  ...
  if (balance > 1 && balanceFactor(rootNode->leftChild) >= 0)
  {
  ...
  if (balance < -1 && balanceFactor(rootNode->rightChild) <= 0)
  {
...
void deleteAddress()
{
  ...
  else
  {
    ...
    else
    {
      ...
      // Check for AVL tree property violations and fix them if necessary
      updateHeightAndBalanceFactor(rootNode);
      rootNode = rebalance(rootNode);
    }
  }
...
void displayList(struct address_t *rootNode, int depth, struct address_t *node)
{
  ...
  else
  {
    ...
    printf("%d.%d.%d.%d %s (Depth: %d, Height: %d, BF: %d, Parent: ", *(rootNode->octet), *(rootNode->octet + 1), *(rootNode->octet + 2), *(rootNode->octet + 3), rootNode->alias, depth, (height(rootNode) - 1) == -1 ? 0 : height(rootNode) - 1, rootNode->balance);
